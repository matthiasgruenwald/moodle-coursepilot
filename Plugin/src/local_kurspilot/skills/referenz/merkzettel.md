...
ausschliesslich über die Werkzeuge aus `kurspilot_get_skill("kontextbereich")`,
...
siehe `kurspilot_get_skill("mcp-tools")`) in Originalqualität in den Bestand
...
1. Vor der Codex-Rückfrage nach Shell-Freigabe ankündigen, was gleich
   ...
   Bestand").
2. `kurspilot_create_werkbank_download_links` für **alle** Werkbankpunkte
...
4. Stimmt die lokale Prüfsumme mit dem gemeldeten `sha1` überein: die
   Werkbankdatei per `kurspilot_delete_material_files` löschen — der
...
vorhanden?") scheitert, wenn die Kurspilot-Verbindung nur für ein
