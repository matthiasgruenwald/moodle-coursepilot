...
`coursepilot`, `coursepilot-planen` und `coursepilot-umsetzen`. Detailwissen zu
...
- Drei Adapter: `coursepilot`, `coursepilot-planen`, `coursepilot-umsetzen`.
- Arbeitsdateien (`plan.md`, `status.md`, Journal, Materialnotizen,
  ...
  Werkzeuge und Ablageordnung siehe `coursepilot_get_skill("kontextbereich")`.
...
`coursepilot-umsetzen` prueft `status.md` vor jedem Moodle-Schreibzugriff. Steht
der Status auf `in_planung`, wird keine Schreibaktion ausgefuehrt; Coursepilot
leitet stattdessen transparent zu `coursepilot-planen` fuer Review und Freigabe
...
`coursepilot-planen` klaert Unterrichtseinheit oder Unterthema, liest
...
Wenn ein Moodle-Ziel bekannt ist, liest `coursepilot-planen` den Kursstand ueber
...
benennt `coursepilot-planen` das ausdruecklich als Werkzeugluecke, statt zu
...
`coursepilot-umsetzen` setzt nur freigegebene Plaene um. Bei `in_planung`
startet er keine Moodle-Schreibaktion, sondern benennt den Wechsel zu
`coursepilot-planen` fuer Review und Freigabe. Nach Moodle-Schreibzugriffen
...
`coursepilot-umsetzen` die Verschiebung vor dem Moodle-Schreibzugriff im Journal
...
`coursepilot_update_question_category` zeigt `coursepilot-planen` beziehungsweise
`coursepilot-umsetzen` immer Quelle, Ziel und betroffene Kategorien
...
der Hauptsession und werden erst nach Vorschau/Freigabe an `coursepilot-umsetzen`
delegiert.
...
Vorschau/Freigabe oder als Planrevision zurueck in `coursepilot-planen`. Grosse
