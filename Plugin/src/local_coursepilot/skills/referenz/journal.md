...
2. Fehlt der noetige Kontext, nicht still ohne Gedaechtnis weiterarbeiten:
   ...
   **Erklaerendes Setup** mit Vorschau anbieten (siehe
   `coursepilot_get_skill("kontext-onboarding")`). Nach Bestätigung werden die
...
Kontextwurzel (siehe "Ablageordnung" in `coursepilot_get_skill("kontextbereich")`):
...
`coursepilot_get_skill("implementierungsplan-workflow")`) wird automatisch ein
