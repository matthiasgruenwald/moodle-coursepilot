...
`coursepilot_get_skill("kontextbereich")` gelesen und geschrieben.
...
Kurz anbieten, wie es weitergeht: jetzt planen (`coursepilot-planen`), einen
bereits freigegebenen Plan umsetzen (`coursepilot-umsetzen`) oder spaeter
weiterarbeiten.
...
`coursepilot_get_skill("kontextbereich")`) und wird von der Sachdatei aus
sichtbar verlinkt.
