...
`coursepilot_get_skill("fragetypen")`) — "Anhängen" meint hier den Inhalt, nicht
...
- `coursepilot_list_skills` meldet zu Sitzungsbeginn offene Einträge im Feld
  ...
  erst** `coursepilot_dismiss_ausstand` aufrufen — nie ohne ausdrückliches
  Wort der Lehrkraft.
...
dieser Sitzung abgeschlossen, kein offener Blocker) ruft `coursepilot-umsetzen`
...
(`materialpathiskontext`), die auf `coursepilot_list_context_files`/
