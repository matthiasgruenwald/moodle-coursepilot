...
Lies diese Datei beim Erstellen von Textseiten (`coursepilot_create_module` mit
...
sind (siehe Planstrenge in `coursepilot_get_skill("coursepilot-core")`). Wenn eine
schlichtere Darstellung denselben Zweck erfuellt, ist sie die richtige Wahl.

## Geplanter Abschnittseinstieg (optional fuer coursepilot_update_section, Feld "summary")
...
("Allgemeines"): fachlicher Inhalt ja, automatischer Coursepilot-Prozesscontainer
nein.
...
## Phasen-Header (fuer coursepilot_create_module mit modname="label")
...
## Textseite mit Syntax-Highlighting (fuer coursepilot_create_module mit modname="page")
...
## Aufgabe (fuer coursepilot_create_module mit modname="assign", Feld "intro")
...
`coursepilot_get_skill("interaktive-elemente")`. Fuer Zeichenaufgaben siehe
`coursepilot_get_skill("zeichen-canvas")`.
